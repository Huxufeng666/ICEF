...
# class FPNUNetV3_CoordAttention(nn.Module):
#     def __init__(self, in_ch=1, out_ch=1, features=[64, 128, 256, 512]):
#         super().__init__()
#         self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
...
#         self.up4 = nn.ConvTranspose2d(128, 128, kernel_size=2, stride=2)
#         self.coord4 = CoordinateAttention(128)
#         self.dec4 = ConvBlock(128, 128)

#         self.up3 = nn.ConvTranspose2d(128, 128, kernel_size=2, stride=2)
#         self.coord3 = CoordinateAttention(128)
#         self.dec3 = ConvBlock(128, 128)

#         self.up2 = nn.ConvTranspose2d(128, 128, kernel_size=2, stride=2)
#         self.coord2 = CoordinateAttention(128)
#         self.dec2 = ConvBlock(128, 128)

#         self.up1 = nn.ConvTranspose2d(128, 128, kernel_size=2, stride=2)
#         self.coord1 = CoordinateAttention(128)
#         self.dec1 = ConvBlock(128, 128)
...
#         e2 = self.enc2(self.pool(e1))
#         e3_input = self.pool(e2)
#         e1_resized = F.interpolate(self.e1_encoder_adapter(e1), size=e3_input.shape[2:], mode='bilinear', align_corners=True)
#         e3 = self.enc3(e3_input + e1_resized)
#         e4_input = self.pool(e3)
#         e2_resized = F.interpolate(self.e2_encoder_adapter(e2), size=e4_input.shape[2:], mode='bilinear', align_corners=True)
#         e4 = self.enc4(e4_input + e2_resized)
#         b = self.bottleneck(self.pool(e4))
...
# # Coordinate Attention with Dropout and GELU
# class CoordinateAttention(nn.Module):
#     def __init__(self, in_channels, reduction=32, dropout_prob=0.1):
#         super().__init__()
#         self.pool_h = nn.AdaptiveAvgPool2d((None, 1))
#         self.pool_w = nn.AdaptiveAvgPool2d((1, None))
#         mip = max(8, in_channels // reduction)
...
#         self.conv1 = nn.Conv2d(in_channels, mip, kernel_size=1, stride=1, padding=0)
#         self.bn1 = nn.BatchNorm2d(mip)
#         self.act = nn.GELU()  # Use GELU for smoother gradients
#         self.dropout = nn.Dropout(dropout_prob)  # Add Dropout for regularization
...
#         x_w = self.pool_w(x).permute(0, 1, 3, 2)
#         y = torch.cat([x_h, x_w], dim=2)
#         y = self.dropout(self.act(self.bn1(self.conv1(y))))
#         x_h, x_w = torch.split(y, [h, w], dim=2)
#         x_w = x_w.permute(0, 1, 3, 2)
#         a_h = self.conv_h(x_h).sigmoid()
#         a_w = self.conv_w(x_w).sigmoid()
...
# # ConvBlock with Depthwise Separable Convolution and GELU
# class ConvBlock(nn.Module):
...
#         )
#         self.bn1 = nn.BatchNorm2d(out_ch)
#         self.relu = nn.GELU()  # Use GELU instead of ReLU
...
# # TopDownFPN with Weighted Feature Fusion
# class TopDownFPN(nn.Module):
#     def __init__(self, in_channels_list, out_channels):
#         super().__init__()
#         self.lateral_convs = nn.ModuleList([
...
#         self.fusion_weights = nn.ParameterList([nn.Parameter(torch.ones(1)) for _ in in_channels_list])

#     def forward(self, features):
#         features = features[::-1]  # Reverse for top-down
#         lateral_features = [conv(f) for conv, f in zip(self.lateral_convs, features)]
#         fpn_outputs = []
#         x = lateral_features[0]
#         fpn_outputs.append(self.smooth_convs[0](x))
#         for i in range(1, len(lateral_features)):
#             x = F.interpolate(x, size=lateral_features[i].shape[2:], mode='nearest') + self.fusion_weights[i] * lateral_features[i]
#             fpn_outputs.append(self.smooth_convs[i](x))
#         return fpn_outputs[::-1]

# # Main Model with CLIP Integration
# class FPNUNetV3_CoordAttention_1(nn.Module):
#     def __init__(self, in_ch=1, out_ch=1, features=[64, 128, 256, 512]):  # out_ch=3 for RGB output
#         super().__init__()
# # nationality
#         self.pool = nn.MaxPool2d(kernel_size=2, stride=2)

#         # Load CLIP for text-to-image tasks


#         self.clip_model = CLIPModel.from_pretrained("openai/clip-vit-base-patch32")
#         self.clip_processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32")
#         self.text_projection = nn.Linear(512, features[0])  # Map CLIP embedding to feature space
...
#         # If text is provided, use CLIP to encode it
#         if text is not None:
#             inputs = self.clip_processor(text=text, return_tensors="pt", padding=True)
#             text_embedding = self.clip_model.get_text_features(**inputs)  # [batch, 512]
#             text_feature = self.text_projection(text_embedding)  # Map to feature space
#             # Broadcast text feature to match input spatial dimensions
#             text_feature = text_feature.view(-1, text_feature.size(-1), 1, 1).expand(-1, -1, x.size(2), x.size(3))
#             x = x + text_feature  # Combine text and image features
...
# # model = FPNUNetV3_CoordAttention_1(in_ch=1, out_ch=1)
# # x = torch.randn(1, 1, 256, 256)
# # out = model(x)
# # print(out.shape)  # torch.Size([1, 1, 256, 256])
